...
4. 클라이언트: POST /api/toss/auth/generate-token 호출
   ...
   ↓
5. 클라이언트: GET /api/toss/user/me/decrypted 호출
   → 사용자 정보 조회 (복호화된 상태)
   → 서버에서 자동으로 Supabase에 저장/업데이트
   ↓
6. (선택) 클라이언트: GET /api/users/me 호출
   → Supabase DB에서 사용자 정보 조회
```

### 중요 사항

- **자동 저장**: `GET /api/toss/user/me/decrypted` 호출 시 서버에서 자동으로 Supabase에 사용자 정보가 저장됩니다.
- **하위 호환성**: 기존 API 동작은 그대로 유지되므로 클라이언트 코드 변경 없이 사용 가능합니다.
- **DB 조회**: Supabase에 저장된 사용자 정보를 조회하려면 `/api/users/me` 엔드포인트를 사용하세요.
...
### 1. 사용자 정보 조회 및 복호화 (자동 저장)

**엔드포인트**: `GET /api/toss/user/me/decrypted`

**설명**:

- 토스 API에서 사용자 정보를 조회하고 복호화합니다.
- **자동으로 Supabase에 저장/업데이트됩니다.**
- 기존 API와 동일한 응답을 반환합니다.
...
- 이 엔드포인트를 호출하면 서버에서 자동으로 Supabase에 사용자 정보가 저장됩니다.
- Supabase 저장 실패 시에도 기존 응답은 정상적으로 반환됩니다 (에러는 로그에만 기록).
- 클라이언트 코드 변경 없이 기존처럼 사용하면 됩니다.
...
- Supabase DB에서 사용자 정보를 조회합니다.
- 사용자가 DB에 없으면 토스에서 가져와서 저장한 후 반환합니다.
...
// 3. 사용자 정보 조회 (자동으로 Supabase에 저장됨)
const userResponse = await apiClient.get('/api/toss/user/me/decrypted', {
  headers: {
    Authorization: `Bearer ${accessToken}`,
  },
})

const tossUserInfo = userResponse.data.data
// { userKey, scope, agreedTerms, name, phone, ... }

// 4. (선택) Supabase에서 사용자 정보 조회
const dbUserResponse = await apiClient.get('/api/users/me', {
  headers: {
    Authorization: `Bearer ${accessToken}`,
  },
})

const dbUser = dbUserResponse.data.data
...
### 패턴 1: 토스 정보만 사용 (기존 방식)

```typescript
// 기존 코드 그대로 사용
const response = await apiClient.get('/api/toss/user/me/decrypted')
const userInfo = response.data.data
// 자동으로 Supabase에 저장됨 (백그라운드)
```

### 패턴 2: Supabase 정보 사용 (권장)

```typescript
// Supabase에서 조회 (없으면 자동으로 토스에서 가져와서 저장)
const response = await apiClient.get('/api/users/me')
const user = response.data.data
// { id, createdAt, updatedAt, lastLoginAt 등 추가 정보 포함 }
```
...
```typescript
// 토스 정보로 즉시 UI 업데이트
const tossInfo = await apiClient.get('/api/toss/user/me/decrypted')

// Supabase 정보로 추가 기능 사용 (닉네임, 설정 등)
const dbUser = await apiClient.get('/api/users/me')
```
...
1. **자동 저장**: `GET /api/toss/user/me/decrypted` 호출 시 자동으로 Supabase에 저장됩니다.
2. **토큰 관리**: AccessToken은 localStorage에 저장하고 모든 API 요청에 포함해야 합니다.
3. **에러 처리**: Supabase 저장 실패 시에도 기존 응답은 정상 반환되므로, 클라이언트는 별도 처리 불필요.
